...
class OrderItem(models.Model):
    ...
    def __str__(self):
        return f"{self.product.title} (x{self.quantity}) - {self.price}zł"
...
class Order(models.Model):
    ...
    # Payment & Shipping
    billing_method = models.CharField(max_length=25, null=False, blank=False)
    shipping_method = models.CharField(max_length=15, null=False, blank=False)

    # Guest user details (for non-logged-in users)
    ...
    guest_city = models.CharField(max_length=50, null=True, blank=True)
    guest_zip_code = models.CharField(max_length=10, null=True, blank=True)
    ...
    def __str__(self):
        return f"Zamówienie {self.id} - {self.user if self.user else self.guest_email}"
